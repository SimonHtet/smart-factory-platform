-- ============================================================
-- TRI_UPDATE_FILLER_V5.8  ** COMMS-AUDIT INSTRUMENTED COPY **
-- Database : DB_BUDIBASE
-- Table    : T_M_Filler_Process
-- Author   : Simon (DairyPlus Manufacturing Systems Engineer)
--
-- !!! TEMPORARY DIAGNOSTIC BUILD (2026-07-02) !!!
-- -------------------------------------------------------
-- Identical to pipeline/sql/TRI_UPDATE_FILLER_V5.8.sql PLUS a
-- comms-audit logging block at the top of the trigger body
-- (search for "TEMP COMMS AUDIT"). It logs HEARTBEAT / NULLDATA /
-- MULTIROW events to dbo.Comms_Audit to prove whether OPMS is
-- delivering incomplete/bundled data during the step-13 data-loss
-- incidents (started ~2026-06-30 on G3/G1/F3/M1).
--
-- NO production logic is changed. The audit block is wrapped in
-- its own TRY/CATCH so a diagnostics failure can never block an
-- OPMS write.
--
-- Requires : troubleshooting/COMMS_AUDIT_SETUP.sql run first.
-- Read with: troubleshooting/COMMS_AUDIT_QUERIES.sql.
-- Remove by: re-running pipeline/sql/TRI_UPDATE_FILLER_V5.8.sql.
-- ============================================================
--
-- WHAT CHANGED IN V5.8  (2026-06-23)
-- -------------------------------------------------------
-- DE-LINE downtime tracking (signal_DE_NotReady: 0 = OK, 1 = down).
--
-- A DE-line stall drags the TBA filler into downtime that is NOT the
-- filler's own fault. We capture each DE-down episode independently so
-- the analytic layer can isolate it:
--     tba_actual_downtime = total_downtime - de_downtime  (floored at 0)
--
-- Pure binary edge (no step machine, no segments, no abort):
--   signal_DE_NotReady 0 -> 1  START : DE_Downtime_Count++,
--                                       Current_DE_Downtime_Start = now,
--                                       insert DE_Downtime_log row Status='OPEN'.
--   signal_DE_NotReady 1 -> 0  END   : close the OPEN row with
--                                       Duration_Seconds, add it to
--                                       Total_DE_Downtime_Seconds, clear
--                                       Current_DE_Downtime_Start.
--
-- Accumulates onto [Change paper brik] exactly like Total_Downtime_Seconds,
-- so temp_production_run reads it the same way at the next splice/13/14.
...
USE [DB_BUDIBASE]
GO
CREATE OR ALTER TRIGGER [dbo].[TRI_UPDATE_FILLER_V5_8]
...
BEGIN
    ...
    -- =========================================================
    -- [TEMP COMMS AUDIT 2026-07-02] -- REMOVE when investigation
    -- closes (re-run the canonical V5.8 file).
    -- Pure logging, no production logic. Own TRY/CATCH so a
    -- diagnostics error can never block the OPMS write.
    -- =========================================================
    BEGIN TRY
        -- 1. MULTIROW : one statement touched >1 machine row
        --    (suspected reconnect flush -- the shape that lets one
        --    machine's step 13 stamp other machines' batches).
        IF (SELECT COUNT(*) FROM inserted) > 1
        BEGIN
            DECLARE @Audit_Probe NVARCHAR(400) = ''
            SELECT @Audit_Probe = @Audit_Probe + '|' + ISNULL(Machine, '?') + ':'
                                + ISNULL(CAST(Machine_Step_No AS NVARCHAR(5)), 'NULL')
            FROM inserted

            INSERT INTO dbo.Comms_Audit (Event, Detail)
            VALUES ('MULTIROW',
                    CAST((SELECT COUNT(*) FROM inserted) AS NVARCHAR(10)) + @Audit_Probe)
        END

        -- 2. NULLDATA : machine mid-production (step 8-13) arrived
        --    with an empty counter payload = OPMS delivered the step
        --    but not the counts. Rate-limited to 1/min/machine.
        INSERT INTO dbo.Comms_Audit (Event, Machine, Step, Infeed, Outfeed, Infeed_DE)
        SELECT 'NULLDATA', i.Machine, i.Machine_Step_No,
               i.counter_infeed, i.counter_outfeed, i.counter_infeed_DE
        FROM inserted i
        WHERE i.counter_infeed IS NULL
          AND i.Machine_Step_No BETWEEN 8 AND 13
          AND NOT EXISTS (SELECT 1 FROM dbo.Comms_Audit a
                          WHERE a.Machine = i.Machine
                            AND a.Event   = 'NULLDATA'
                            AND a.Log_Time > DATEADD(SECOND, -60, GETUTCDATE()))

        -- 3. HEARTBEAT : one row per machine per minute while OPMS
        --    is writing. A gap in heartbeats during production =
        --    OPMS went silent for that machine (comms drop).
        INSERT INTO dbo.Comms_Audit (Event, Machine, Step, Infeed, Outfeed, Infeed_DE)
        SELECT 'HEARTBEAT', i.Machine, i.Machine_Step_No,
               i.counter_infeed, i.counter_outfeed, i.counter_infeed_DE
        FROM inserted i
        WHERE NOT EXISTS (SELECT 1 FROM dbo.Comms_Audit a
                          WHERE a.Machine = i.Machine
                            AND a.Event   = 'HEARTBEAT'
                            AND a.Log_Time > DATEADD(SECOND, -60, GETUTCDATE()))
    END TRY
    BEGIN CATCH
        INSERT INTO t_log(txt) VALUES ('COMMS_AUDIT_ERR:' + ERROR_MESSAGE())
    END CATCH
    -- ================== [END TEMP COMMS AUDIT] ==================
    ...
    BEGIN TRY
        ...
        -- -------------------------------------------------------
        -- [V5.8] DE LINE DOWN START : signal_DE_NotReady 0 -> 1
        -- Open a DE downtime episode and stamp the start on the batch.
        -- All machines. Guarded against a duplicate OPEN per batch.
        -- -------------------------------------------------------
        IF EXISTS (
            SELECT 1 FROM inserted i
            JOIN deleted d ON i.Machine = d.Machine
            ...
            BEGIN
                ...
                BEGIN
                    ...
                    INSERT INTO [DE_Downtime_log]
                        (Machine, Batch_ID, Start_Time, End_Time, Duration_Seconds, Status, Log_Time)
                    VALUES
                        (@cur_Machine_DE, @GID_DE, GETUTCDATE(), NULL, NULL, 'OPEN', GETUTCDATE())
        ...
        -- -------------------------------------------------------
        -- [V5.8] DE LINE DOWN END : signal_DE_NotReady 1 -> 0
        -- Close the OPEN episode, stamp Duration_Seconds, and add it
        -- to Total_DE_Downtime_Seconds on the batch. Falls back to the
        -- OPEN row's batch if [end time] was set mid-episode.
        -- -------------------------------------------------------
        IF EXISTS (
            SELECT 1 FROM inserted i
            JOIN deleted d ON i.Machine = d.Machine
