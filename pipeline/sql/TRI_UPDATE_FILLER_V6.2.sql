...
--   2. DROP TRIGGER [dbo].[TRI_UPDATE_FILLER_V6_1]   -- avoid double-fire
--   3. Run this script (creates TRI_UPDATE_FILLER_V6_2).
...
USE [DB_BUDIBASE]
GO
CREATE OR ALTER TRIGGER [dbo].[TRI_UPDATE_FILLER_V6_2]
...
BEGIN
    ...
    BEGIN TRY
        ...
        -- -------------------------------------------------------
        -- [V6.2] LEAVING STEP 11 : stash the feed counters (A/B/D/M)
        ...
        -- for never happens.
        --
        -- ALL exits from 11, not just ->8 and ->7: a straight 11->0
        -- must be covered too.
        --
        -- OVERWRITES every time -- deliberately NOT guarded by
        -- "IS NULL" the way BigDT_Pending_Start is. A mini downtime
        -- early in the batch would otherwise pin the stash to a
        -- stale low counter that a real outage hours later commits.
        -- Mini downtimes never reach step 0, so an uncommitted stash
        -- is harmless; it is simply replaced by the next exit.
        --
                    ...
                    BEGIN
                        ...
                        BEGIN
                            ...
                            INSERT INTO [Feed_Segment_log]
                                (Machine, Batch_ID, Segment_No, In_Feed_Seg, Out_Feed_Seg, Reset_Time, Ended_By, Log_Time)
                            VALUES
                                (@cur_Machine_Z0, @GID_Z0, @SegNo_Z0, @PendIn_Z0, @PendOut_Z0,
