...
-- -------------------------------------------------------
-- Step 13 end_time re-log for A/D/M machines.
-- Previously [end time] was written once and locked (guard:
-- [end time] IS NULL). For A/D/M the batch stays open until
-- CIP completes (End_time_CIP IS NULL), so a second Step 13
-- signal before CIP was silently ignored, leaving a stale
-- end_time and stale counter snapshot.
-- Fix: for A/D/M, allow Step 13 to re-stamp [end time] and
-- re-read counters as long as End_time_CIP IS NULL.
-- F/G/H/K are unchanged — write-once guard ([end time] IS NULL).
--
-- WHAT CHANGED IN V5.3
-- -------------------------------------------------------
-- Fixed End_time_CIP bug introduced in V5.2.
-- V5.2 was writing [end time] (Step 13 timestamp) into
-- End_time_CIP instead of the actual CIP completion time.
-- Fix: SET End_time_CIP = GETUTCDATE() — stamps real CIP time.
-- @EndTime_S14 ([end time]) is kept in SELECT for t_log only.
--
...
USE [DB_BUDIBASE]
GO
CREATE OR ALTER TRIGGER [dbo].[TRI_UPDATE_FILLER_V5_4]
